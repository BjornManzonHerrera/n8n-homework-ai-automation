...
# List of .env files for different accounts
ENV_FILES = [".env.account1", ".env.account2", ".env.account3"]  # Add more as needed
CURRENT_ACCOUNT_INDEX = 0
USAGE_THRESHOLD = 900  # Switch keys when daily usage reaches 90% of 1,000 requests
...
]  # Service account credentials for monitoring

def get_api_usage(credentials_path):
...
def switch_api_key():
    ...
    print(f"Switched to API key from {ENV_FILES[CURRENT_ACCOUNT_INDEX]}")
    with open("current_key.json", "w") as f:
        json.dump({"current_env": ENV_FILES[CURRENT_ACCOUNT_INDEX]}, f)
...
def check_and_rotate_keys():
    ...
    current_credentials = CREDENTIALS_PATHS[CURRENT_ACCOUNT_INDEX]
    usage = get_api_usage(current_credentials)
    print(f"Current API usage: {usage} requests")
...
def load_current_key():
    ...
    try:
        with open("current_key.json", "r") as f:
            ...
            print(f"Loaded API key from {current_env}")
